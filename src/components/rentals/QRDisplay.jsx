...
export function QRDisplay({ rental, onClose }) {
  ...
  return (
    <div className="flex flex-col items-center">
      {/* Exito header */}
      <div className="w-14 h-14 rounded-2xl bg-success-soft flex items-center justify-center mb-3">
      ...
      {/* QR Code */}
      <div
        className="p-5 rounded-2xl border border-border mb-5"
        style={{
      ...
      {/* Rental info card */}
      <div className="w-full rounded-2xl border border-border bg-bg-card p-4 space-y-3 mb-5"
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-sm font-semibold text-text">{tier?.name || rental.type}</span>
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-text-muted">Estacion:</span>
          <span className="font-medium text-text">{rental.stationName || rental.stationId}</span>
        </div>

        {/* Hora de retiro */}
        <div className="flex items-center gap-2 text-sm">
          <Clock size={14} className="text-accent shrink-0" />
          <span className="text-text-muted">Retiro:</span>
          <span className="font-medium text-text">{formatDateTime(rental.pickupTime)}</span>
        </div>
      </div>
